...
| `footstep [--set <editorID>] [--armature <formid-or-edid>]` | walk the footstep chain read-only: record counts (`FSTS`/`FSTP`/`IPDS`/`IPCT`/`ARMA` with `SNDD`), the set chosen, then per gait every `FSTP` in that gait's list as `tag: footstep -> impact -> sound file`, with unresolvable links reported rather than skipped; defaults to `DefaultFootstepSet`, `--set` names another by editor ID, and `--armature` reads the set off an `ARMA`'s `SNDD` the way an actor's boots do ([footstep records](/formats/footstep.md)) |
