...
| M16.7 combat AI | `World > Combat & Physics > Combat Loop` | `Destination-combatPhysics` | `CombatHostilityControl`, `CombatClearTraceControl` | `CombatLoopStatsLabel` (one line per fighting actor) | `CombatBehaviorMachineTests`, `CombatBehaviorRetreatTests`, `CombatLoopRuntimeTests`, `CombatLoopDisengageTests`, `CombatLoopStateTests`, `CombatPhysicsPanelTests`, `ConditionActorFunctionTests`, `PapyrusNativeActorCombatTests`, `DestinationRegistryTests`, `CombatLoopRealDataTests` (env-gated, `make realtest`) | [combat loop](/engine/combat.md) |
