...
| M14 footstep audio | `World > Audio > Footsteps` | `Destination-audio` | `AudioFootstepsEnabledControl`, `AudioFootstepTagControl`, `AudioPlayFootstepControl`, `AudioEnabledControl` (the engine the director needs), `CameraMovementModeControl` (the walk mode that fires the events) | `AudioFootstepsStatsLabel` | `AudioFootstepsPanelTests`, `WorldAudioFootstepDirectorTests`, `FootstepStoreTests`, `FootstepRecordTests`, `WAVFileTests`, `LocomotionBridgeEventDrainTests`, `DestinationRegistryTests`, `FootstepRealDataTests` (env-gated, `make realtest`) | [terrain walk mode](/engine/walk-mode.md) |
