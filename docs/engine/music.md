...
<!-- Stage 4 (panel + acceptance) fills this in. Placeholder on purpose. -->

The sidebar section and the acceptance record required by the
[sidebar verification convention](/tools/sidebar-acceptance.md) are added with
the World > Audio > Music panel. The provider surface the panel binds to is
already in place on `AudioControlProviding`:

* `musicEnabled` — playback toggle.
* `selectableMusicTypeNames` — sorted MUSC editor ids for the picker.
* `forceMusicType(named:)` — crossfades to a chosen playlist, bypassing the
  precedence chain; returns nil or a failure reason.
* `stopMusic()` — force-stops; the next cell change starts music again.
* `currentMusicDescription`, `currentMusicStateName`, `currentMusicTrackName`,
  `lastMusicError` — readout inputs.
