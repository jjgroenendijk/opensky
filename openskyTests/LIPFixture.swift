...
// the documented 33-slot frame grid; no game-derived bytes are fixtures.
...
enum LIPFixture {
    ...
    static func file(
        ...
        data.appendUInt32(durationTicks ?? UInt32(frameCount) * 132 + 28)
        ...
        append(cells: cells, to: &data)
        return data
    }

    private static func append(cells: [Cell], to data: inout Data) {
        ...
        for (index, cell) in sorted.enumerated() {
            ...
            let gap = sorted[index + 1].position - cell.position - occupied
