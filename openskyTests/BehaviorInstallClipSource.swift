// The install-backed `BehaviorClipSource` the env-gated behavior tests share
// (issues #187 and #330). Loads clips out of the user's own install on demand,
// so only the clips the graphs actually reach are read, and caps the cache so a
// graph that reaches thousands of them cannot pull the whole animation set into
// memory.
//
...
final class InstallBehaviorClipSource: BehaviorClipSource, @unchecked Sendable {
